...
# Firebase setup (use local firebasekey.json)
...
def export_pdf():
    try:
        ...
        for r in data:
            created = (
                r["createdAt"].strftime("%Y-%m-%d %H:%M:%S")
                if r["createdAt"]
                else ""
            )
            table_data.append(
...
def control_motor():
    try:
        ...
        if action == "off":
            ...
            return jsonify(
                {"status": "success", "message": "Motor turned ON at 50%"},
            ), 200
            ...
            return jsonify(
                {
                    "status": "success",
                    "message": f"Speed set to {rpm_value}%",
                }
            ), 200
...
if __name__ == "__main__":
    # Listen on all network interfaces so ESP32 and other devices can reach it
